...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec2 hmm = (uv * iResolution.y + iResolution.xy) / (2.0 * iResolution.xy);
    vec4 originalColor = texture(iChannel0, hmm);
    
    vec2 mo = vec2(iTime * 0.01, cos(iTime * 0.025) * 3.0);
    ...
    vec3 bg = mix(vec3(0.3, 0.5, 0.8), vec3(0.4, 0.5, 0.6), 1.0 - (uv.y + 1.0) * 0.5);
    color.rgb += bg;

    float threshold = 0.1;
    float objectMask = step(threshold, max(max(originalColor.r, originalColor.g), originalColor.b));
    color.rgb = mix(color.rgb, originalColor.rgb, objectMask);
