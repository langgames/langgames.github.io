...
class StaticGrid extends EngineObject {
    constructor(rows, cols, words, margin, colors = {}) {
        super()
        this.rows = rows;
        this.cols = cols;
        this.words = words;
    ...
    clearShowReading() {
        this.showReading = new Arrayy(rows * cols).fill(false)
    }

    draw() {
        const cellSize = vec2(4, 4);
        const totalSize = vec2(
            (this.cols - 1) * (cellSize.x + this.margin),
            (this.rows - 1) * (cellSize.y + this.margin)
        );
        const startPos = cameraPos.subtract(totalSize.scale(0.5));

        for (let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
                const index = i * this.cols + j;
                const pos = startPos.add(vec2(
                    j * (cellSize.x + this.margin),
                    totalSize.y - i * (cellSize.y + this.margin)
                ));
                const isSelected = index === selectedIndex;

                drawRect(pos, cellSize, isSelected ? this.selectedCellColor : this.cellColor);
                if (!this.masked[index]) {
                    if (this.showReading[index])
                        drawText(this.words[index].reading, pos.add(vec2(0, cellSize.y * 0.25)), 0.6, isSelected ? this.selectedReadingTextColor : this.readingTextColor);
                    drawText(this.words[index].word, pos, 1, isSelected ? this.selectedTextColor : this.textColor);
                } else {
                    drawText(this.maskIcon, pos, 1, isSelected ? this.selectedMaskColor : this.maskColor);
                }
            }
        }
    }
