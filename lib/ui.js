...
class StaticGrid extends EngineObject {
    constructor(rows, cols, words, margin) {
        super()
        this.rows = rows;
        this.cols = cols;
        this.words = words;
    ...
    draw() {
        ...
        for (let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
                ...
                drawRect(pos, cellSize, isSelected ? PALETTE.green : PALETTE.darkGray);
                if (!this.masked[index]) {
                    if (this.showReading[index])
                        drawText(this.words[index].reading, pos.add(vec2(0, cellSize.y * 0.25)), 0.6, isSelected ? PALETTE.darkGray : PALETTE.white);
                    drawText(this.words[index].word, pos, 1, isSelected ? PALETTE.darkGray : PALETTE.white);
                } else {
                    drawText(this.maskIcon, pos, 1, isSelected ? PALETTE.darkGray : PALETTE.white);
                }
            }
        }
    }
