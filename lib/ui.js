...
class StaticGrid extends EngineObject {
    ...
    update() {
        ...
        const totalSize = vec2(
            (this.cols - 1) * (this.cellSize.x + this.margin),
            (this.rows - 1) * (this.cellSize.y + this.margin)
        );
        ...
        for (let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
                ...
                const pos = startPos.add(vec2(
                    ...
                    totalSize.y - i * (this.cellSize.y + this.margin)
                ));
    ...
    render() {
        ...
        for (let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
                ...
                drawRect(pos, this.cellSize, isSelected ? this.selectedCellColor : (isHovered ? this.selectedCellColor.lerp(this.cellColor, 0.5) : this.cellColor));
                if (!this.masked[index]) {
                    if (this.showReading[index])
                        drawText(this.words[index].reading, pos.add(vec2(0, this.cellSize.y * 0.25)), 0.6, isSelected ? this.selectedReadingTextColor : this.readingTextColor);
